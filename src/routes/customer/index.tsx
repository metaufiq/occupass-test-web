...
const CustomerList = ({ onSelectCustomer }: Props) => {
  ...
  useEffect(() => {
    setLoading(true);
    fetchAllCustomers({ skip, take: PAGE_SIZE })
      .then((response) => {
        if (
          ...
          setSkip((prev) => prev + response.response!.results.length);
        }
      })
      .finally(() => {
        setLoading(false);
  ...
  const handlePageChange = useCallback((page: number) => {
    ...
    fetchAllCustomers({ skip: newSkip, take: PAGE_SIZE })
      .then((response) => {
        if (response?.response?.results) {
          setCustomers(response.response.results);
          setHasMorePages(response.response.results.length === PAGE_SIZE);
        }
      })
      .finally(() => {
        setLoading(false);
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader
        title="Customers"
        desc="Manage your customer database with ease"
      />
      {!loading && (
        <ControlBar
